...
\end{subfigure}
\caption{The ROC of classifiers that use hand chosen keywords (a) and algorithmically chosen keywords (b) to determine if an individual is ill. The top 10 (solid line), 100 (dashed line) and 1000 (double dashed line) were selected as the features. }
\label{fig:roc_keyword}
\end{figure}

Also algorithmically choose keywords, start by processing most common 12393 (5000 plus ties) stemmed words. Rank keywords by information gain, with 10 fold cv. Then select \{10,100,1000\} most relevant keywords and to ML on them.
